...
class User(AbstractUser):
    age = models.IntegerField(
        validators=[MinValueValidator(1), MaxValueValidator(130)], null=True, blank=True
    )
...
class BusRoute(models.Model):
    ...
    direction = models.CharField(
        max_length=10, choices=[("go", "Lượt đi"), ("return", "Lượt về")]
    )
...
class RouteStation(models.Model):
    route = models.ForeignKey(
        BusRoute, on_delete=models.CASCADE, related_name="route_stations"
    )
    station = models.ForeignKey(
        BusStation, on_delete=models.CASCADE, related_name="station_routes"
    )
